    ...
    public PathfindingSnapshot extendSearch(int maxToSettle) {
        ...
        while (!frontier.isEmpty() && settledCount < maxToSettle){
            ...
            if (!settledIds.get(currentId)){
                ...
                VertexType v = graph.getVertex(currentId);
                for (EdgeType e : v.outgoingEdges()){
                    int neighborId = e.endId();
                    int dist = distances[currentId] + weigher.weight(e);
                    if (distances[neighborId] == -1 || dist< distances[neighborId]){
        ...
        return new PathfindingSnapshot(
                startId, Arrays.copyOf(distances, distances.length),
