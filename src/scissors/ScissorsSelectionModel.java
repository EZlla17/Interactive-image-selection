...
import selector.PolyLineBuffer;
...
public class ScissorsSelectionModel extends SelectionModel {
    ...
    protected void appendToSelection(Point p) {
        ...
        PolyLine line = graph.pathToPolyLine(path);
        segments.add(line);
    ...
    private class ShortestPathsWorker
        ...
        public PathfindingSnapshot doInBackground() {
            PathfindingSnapshot finalSnapshot = null;
            while (!pathfinder.allPathsFound()){

                if (isCancelled()) {
                    return null;
                }
                finalSnapshot = pathfinder.extendSearch(10000);
                int settledCount = pathfinder.settledCount();
                int progress = (int) ((double) settledCount / pathfinder.vertexCount() * 100);
