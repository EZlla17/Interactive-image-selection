...
public class ScissorsWeights {
    ...
    public static Weigher<ImageEdge> makeWeigher(String weightName, ImageGraph graph) {
        return switch (weightName) {
            ...
            case "SuperFancyRainbowColorfulWeight" -> new SuperFancyRainbowColorfulWeight(graph);
    ...
    public static Iterable<String> weightNames() {
        return List.of("CrossGradMono", "SuperFancyRainbowColorfulWeight");
    }
    ...
    static class SuperFancyRainbowColorfulWeight implements Weigher<ImageEdge> {
         ...
         * Create a new weigher capable of weighing edges in `graph`.
         */
        SuperFancyRainbowColorfulWeight(ImageGraph graph) {
            this.graph = graph;

            // Extract the "raster" for our graph's image, from which we can query pixel
            //  brightnesses.

            this.src = graph.raster();
        }

        /**
         * Create a new weigher capable of weighing edges in `graph` utilizing the color data.
         */
        @Override
        public int weight(ImageEdge edge) {
            // Get location of pixel at edge's start
            ...
            // Extract RGB values for this pixel and its neighbors
            int red = crossGrad(this.src, x, y, 0, edge.dir());
            int green = crossGrad(this.src, x, y, 1, edge.dir());
            int blue = crossGrad(this.src, x, y, 2, edge.dir());

            // Compute the  Grey Enhancer
            int greyIntensity = getGreyIntensity(red,green,blue);

            // Compute the Grey Enhancer
            double redEnhancer = (255 - red) * 0.5;
            double greenEnhancer = (255 - green) * 0.5;

            // Computer the Red & Green Enhancer
            red = red + (int)redEnhancer;
            green = green - (int)(greenEnhancer);

            // Combine magnified differences
            double validWeight = (red + green + blue) * 0.5 + (greyIntensity * 0.5);

            // Compute the magnitude of the slope perpendicular to this edge, multiplied by this
            //  edge's length, then subtract it from the best possible value.
            return Math.abs(eGradMax - (int)validWeight);
        }

        /** Compute the Grey Intensity of the color composed by red 'r', green 'g' and blue 'b'. */
        private int getGreyIntensity(int r, int g, int b) {
